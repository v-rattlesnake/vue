  ...
  <div id="app">
    <div class="home">
      ...
      <div class="content">
        ...
        <transition>
          <collapse
            @grayscales="grayscaleing"
            @blurCs="blurCing"
            @opacitys="opacitying"
            v-show="setHeight"
            @setBackground="setBackground"
            :class="['animated', setHeight ? 'bounceInDown' : 'bounceOutUp']"
          ></collapse>
        </transition>
        <content-t></content-t>
...
export default {
  ...
  methods: {
    fetchCustomers() {
      this.$axios
        .get(
          "https://free-api.heweather.net/s6/weather/lifestyle?location=历下&key=78b3781ea10742e99d737f4bfdba35d4"
        )
        .then(function(res) {
          console.log(res);
        });
