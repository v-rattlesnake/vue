        ...
        <ul>
            ...
            <li v-for="(val,ind) in musicLists" :key="ind">
                <music-list :ccc="ccc">
                    ...
                    <template v-slot:time="time">3:44</template>
                    <template v-slot:likes="likes">
                    ...
                    <template v-slot:play="play">
                        ...
                        <div class="listCaozuo d-flex d-md-none">
                            ...
                            <pull-down-list :none="index === ind" :style-z="'top:0;left:0;transform:translate(-100%);background:#fff;width:100%;min-width:70px'">
                                <template v-slot:a="a"><btn>播放</btn></template>
                                <template v-slot:b="b"><btn>下架</btn></template>
                                <template v-slot:c="c"><btn>删除</btn></template>
                            </pull-down-list>
                        </div>
                    </template>
                </music-list>
                <transition>
                    <p :class="['animated xiangPage d-sm-none', (xiangqingI === ind ? 'zoomInDown':'zoomOutUp')]" v-show="xiangqingI === ind">
                        <span>作者：{{val.b}}</span>
                        <span>时长：3:11</span>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        musicLists: [{ a: "阿斯顿发", b: "asdfasd", c: "dfasdfasdf", d: "999"},
